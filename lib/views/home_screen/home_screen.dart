  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          // Debug action to refresh student data
          IconButton(
            icon: const Icon(Icons.refresh),
            onPressed: () {
              final provider = Provider.of<StudentProvider>(context, listen: false);
              provider.refreshStudentFromAuth();
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('Refreshing student data...'))
              );
            },
          ),
        ],
      ...
      body: Consumer<StudentProvider>(
        builder: (context, studentProvider, _) {
            ...
            return Center(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 20),
                  ElevatedButton(
                    onPressed: () => studentProvider.refreshStudentFromAuth(),
          ...
          // Continue with original code using the student data
          return FutureBuilder<List<TimetableEntry>>(
            future: getTodaySessions(studentId),
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) {
